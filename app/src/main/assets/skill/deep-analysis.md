...
| eclipsed | 같은 트리거의 다른 제안(choice = 이긴 제안의 signature)에 밀려 조건을 통과하고도 안 떴다 — 반복되면 그 트리거에 **경쟁**이 있다는 뜻. 조건(시간·요일)으로 두 제안의 순간을 갈라 해소하는 것이 정석이고, 그게 안 되면 밀리는 쪽의 정리를 검토하라 |
...
- **확신이 없는 조건은 refinements 로 확정하지 말고 실험으로 검증하라.**
  refinements 는 영구(사용자가 승인 카드에서 [적용할게요]를 누르면 확정),
  실험은 만료가 있다 — 되돌릴 수 있는 쪽이 먼저다.
...
- refinements 는 **즉시 적용되지 않는다** — 앱이 승인 카드([적용할게요/
  그대로 둘게요])로 바꿔 사용자에게 보이고, 탭해야 적용된다. 실험과 같은
  규약이다: 모든 변경은 사용자의 탭을 거친다.
- insight 는 그 승인 카드의 본문이 된다 — 사용자가 이 한 문장만 보고 적용
  여부를 결정한다. **무엇을 근거로 무엇을 바꾸려는지**를 쓴다. 예: "주말
  심야의 거절 2건을 반영해 평일 9–16시로 좁힐게요 — 장 운영 시간과
  일치합니다."
