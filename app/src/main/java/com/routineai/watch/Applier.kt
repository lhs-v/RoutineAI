...
object Applier {
     ...
     * 최근앱 → 고급 옵션 → 분할 화면으로 열기까지 자동으로 가고, **두 번째
     * 앱은 사용자가 고른다**(사용자 결정). 삼성 앱 선택 그리드의 자동 탭은
     * 안정적으로 먹지 않았고, 실패 시 순차 실행 낙하가 "분할이 아니라 화면
     * 전환"으로 보여 더 나빴다. 여기까지 데려다주면 남은 건 탭 한 번이다.
     * 분할 진입 자체가 실패하면 두 앱을 차례로 열어 최소한 둘 다 손에 닿게 한다.
     *
    ...
    private fun splitMessage(split: Boolean, a11y: Boolean, widthDp: Int): String = when {
        split -> "분할 화면을 열었습니다 — 함께 볼 앱을 골라주세요"
