...
- CLI 로만 빌드한다면 SDK 위치를 알려줘야 한다. 저장소 루트에
  `local.properties` 를 만들고 한 줄:
  `sdk.dir=C\:\\Users\\<이름>\\AppData\\Local\\Android\\Sdk` (또는 환경변수
  `ANDROID_HOME`). 이 파일은 `.gitignore` 대상이라 클론에는 없다 —
  없으면 첫 명령이 "SDK location not found" 로 실패한다.
...
4. **절전 예외** — 설정 탭의 "절전 예외" 카드. 실시간 제안을 쓸 거면
